...
I'm not actually being original here; Yeo Kheng Meng in his article
...
If you try to boot a Gentoo install CD in 86Box, as of this writing,
on 86Box emulating a 486, it won't get past GRUB. GRUB's easy to get around, you just
use the `syslinux` family of bootloaders instead, specifically `isolinux`, but...
...
Contemporary 486 motherboards predate El Torito, so they won't boot off CD.
...
into the libc, which propagates into the userland, which causes 486s to "die in pain".[^1]
...
that just barely boots [my specific 86Box config](assets/486-gentoo/86box.cfg)[^4]
...
the Intel iDX4 at 100 MHz is your best bet[^2]. You might need to wait hours (3 hours on the 486DX/25)
...
well as, or better than[^3], a straight RAM-to-RAM copy on modern machines.
...
had to abort my first run because of recording problems.[^4]
...
[^1]: Explanation and phrasing yoinked from [marmolak/gray486linux](https://github.com/marmolak/gray486linux/blob/33e67c720616180b9d3a22ffc4125a9a52018838/releases/hanggai/src/patches/kernel/0001-WIP-patch-kernel-to-skip-endbr32-instruction-on-real.patch).

[^2]: I can't tell if the Cyrix Cx5x86 is or isn't an "i586" in terms of instruction set,
      and I'm not taking my chances with the AMD 586s or the "(enhanced)" 486s, either.

[^3]: The "or better" part is why the PlayStation 5 has a dedicated decompression unit;
...
[^4]: I've had this set to use PCap networking instead of SLiRP; I've had 86Box
...
[^5]: Yes, I *am* thinking of recording (and streaming) my next run.
